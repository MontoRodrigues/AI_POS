...
@Component({
  selector: 'app-pos-external-scanner',
  imports: [FormsModule],
  templateUrl: './pos-external-scanner.html',
  styleUrl: './pos-external-scanner.css'
})
export class PosExternalScanner {
    ...
    const detectAndRepeat = async () => {
      try {
        ...
        if (barcodes.length > 0) {
          if (!this.waitBeforeScanning) {
            console.log("barcodes", barcodes);

            // write the barcode output to device
            if (this.currentDevice() != null) {
              this.waitBeforeScanning = true;
              ...
              setTimeout(() => { this.waitBeforeScanning = false; this.cdRef.detectChanges(); }, 3000);
            }
  ...
  connectionCode: string = "737875"; // remove it once done
  ...
  async ConnectedToDevice() {
    ...
    if (this.machineId != null) {
      ...
      if (d != null) {
        ...
        device.scanner.status = "CONNECTED";
        await this.firebaseService.setDocument(defaultConfig.collections.devices.name + "/" + this.machineId, device);
